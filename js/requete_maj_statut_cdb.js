...
// description : requete serveur pour modifier le statut de chef de bord de la personne
...
function requete_maj_statut_cdb(item_menu, code_membre, nouveau_statut, modal_id) {
  ...
  const envoi = {code: code_membre, cdb: nouveau_statut};

  var xmlhttp = new XMLHttpRequest();
  var url = "php/scripts/membre_statut_cdb_maj.php?";
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      titre_modal.textContent = "Modification statut Chef de bord";
